...
O ApolloBank trata-se de uma aplicação web de um Banco Digital que inclui funcionalidades como armazenamento dos dados dos usuários (registro e login), histórico de transações e tela do usuário.

Esse repositório, da parte Front-End da aplicação, foi a segunda Task do hackadev.
...
Tecnologias de Back-End em construção para as próximas etapas.
...
| Tela de Login | A tela de Login permite o usuário entrar em sua conta já cadastrada. O acesso é feito pelo CPF e a senha de 6 dígitos informados na tela de registro.  |
...
| Tela de Minha Conta | Essa é a tela principal da aplicação, onde o usuário acessa após fazer seu login. Nela, o usuário pode acessar todas as funcionalidades que ainda serão implementadas em nosso site, como PIX, transferências, sistemas de pontos, cartões e a parte de histórico de transação que já está implementada. |
| Tela de Histórico | A tela de histórico permite ao usuário acompanhar seus gastos e ganhos, saber qual o método de pagamento das transações, quem enviou, quem recebeu, quanto foi, etc. tudo em ordem cronológica e com opções de filtros, como filtrar transações do último mês, últimos 6 meses, ou todas e também filtrar por metódo de pagamento ou então procurar via pesquisa. |
| Tela de Ajuda | A tela de ajuda é uma tela adicional que serve apenas para trazer alguns detalhes sobre nosso banco e como nos encontrar. |
...
O projeto foi idealizado usando o Figma, e depois transformado em código.

## 👨🏼‍🏫 Construção, Idealização e Gerência

Criar um projeto para 12 pessoas colaborarem não é algo simples e precisa de um planejamento concreto. A divisão de responsabilidade, e ferramentas para acompanhar o desenvolvimento são cruciais.

Dito isso, como foram solicitadas poucas tarefas, dividimos nossa equipe em duplas e separamos entra as duplas as tarefas.

| Dupla | Responsabilidade |
| --- | --- |
| Carla e Yasmin | HomePage |
| Caio e Cleber | Minha Conta |
| Daiane e Lucas | Registro |
| Brenda e Vitor | Login |
| Luiz e Michele | Histórico de transações |

Para dividir essas responsabilidades, definir prazos e detalhar o funcionamento da tela, usamos o Trello. O Trello é uma ferramenta para gerenciamento de tarefas baseado no método Kanban, de divisão de tarefas. Nele, é possível criar fluxos de progresso (a fazer, fazendo, feito) e criar Cards (tarefas) onde é possível descrever detalhadamente as tarefas e inserir os usuários responsáveis por cada card.

![Trello da equipe](https://github.com/Apollo-Coders/ApolloBank/assets/78491545/837b3e18-3d95-420f-9b1b-d2fa5c19d500)
